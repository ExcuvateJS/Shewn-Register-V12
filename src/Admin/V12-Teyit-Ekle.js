...
if (member.id === message.author.id) return message.channel.send(embed.setDescription(`${message.author} Kendinine kayıt ekleyemezsin!`)).then(shewn => shewn.delete({timeout : 5000})).then(message.react(emoji.redemoj))
...
if(!args[1]) {
    ...
    .setDescription(`Bir değer girmelisin;
    
    .teyitekle @Shewn/ID erkek <miktar>
    .teyitekle @Shewn/ID kadın <miktar>`)
...
if(args[1] === 'erkek') {
let miktar = args[2]
if(!miktar) return message.channel.send(embed.setDescription(`Eklemek istediğinz kadar sayı girin!`)).then(shewn => shewn.delete({timeout : 7000})).then(message.react(emoji.redemoj)) 
...
if(args[1] === 'kadın') {
    ...
    exports.help = {
        help: "kayıt-ekle [Shewn/ID] [erkek/kız] [miktar]",
        cooldown: 10
     }
