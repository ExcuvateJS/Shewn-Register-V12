...
if (!message.member.roles.cache.has(config.roles.booster) && !message.member.hasPermission('ADMINISTRATOR'))
...
if (message.member.roles.highest.position < member.roles.highest.position) return message.channel.send(embed.setDescription(`Bu Kullanıcı Senden Üst Pozisyonda.`)).then(shewn => shewn.delete({timeout : 7000})).then(message.react(emoji.redemoj))   
...
if (member.user.username.includes(taglı)) {
    ...
    exports.conf = {
        ...
        enabled : true,
        guildOnly : false,
        aliases : ["bi","boosterisim","bn","boosternick"], 
         }
    
    exports.help = {
        help: "booster-nick [Shewn/ID] [isim]",
        cooldown: 5
     }