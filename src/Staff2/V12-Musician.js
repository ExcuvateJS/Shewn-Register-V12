...
if(!args[1]) {
    message.channel.send(embed.setDescription(`**[Hata] Yalnış kullanım!**  ${ayarlar.prefix}Musician **ver/al**`)).then(message.react(emoji.redemoj))
    return;    
    }
        ...
        if (args[1] == "ver") {
            ...
            await message.guild.members.cache.get(member.id).roles.add(roller.musician)
      
            message.channel.send(embed2.setDescription(`${member} Adlı Üyeyeye <@&${roller.musician}> Rolü Verildi!`)).then(message.react(emoji.onayemoji))
         }
         ...
         if (args[1] == "al") {
            ...
            await message.guild.members.cache.get(member.id).roles.remove(roller.musician)

            message.channel.send(embed3.setDescription(`${member} Adlı Üyeynin <@&${roller.musician}> Rolü Alındı`)).then(message.react(emoji.onayemoji))
         }
}
exports.conf = {
   name : "musician",
    enabled : true,
    guildOnly : true,
    aliases : ["Musician", "müzisyen", "Müzisyen", "Şarkıcı", "şarkıcı"], 
     }
